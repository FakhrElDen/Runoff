...
// preferences[i][j] is j the preference for voter i
...
bool print_winner(void)
{
    float j = voter_count / 2;
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes > round(j))
        {
            printf("%s\n", candidates[i].name);
            return true;
        }
...
int find_min(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes != 0 && candidates[i].eliminated == false)
        {
